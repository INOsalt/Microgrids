...
class TotalOptimizationManager:
    ...
    def calculate_grid_power_flows(self):
        Pgrid_out = np.zeros(48)
        Pgrid_in = np.zeros(48)
    ...
    def calculate_objective(self):
        ...
        for grid in self.microgrids:
            ...
            # 添加燃气轮机和柴油轮机成本
            for k in range(48):
                if grid.C_sg is not None:
                    Fdown += extracted_values.get(f'Psg_{grid_id}_{k}', 0) * grid.C_sg
...
class Visualization:
    ...
    def plot_microgrid_charts(self, microgrid_id):
        ...
        Pcha_data = -np.array([self.extracted_values.get(f'Pcha_{microgrid_id}_{k}', 0) for k in range(48)])  # 负值
        Pdis_data = -np.array([self.extracted_values.get(f'Pdis_{microgrid_id}_{k}', 0) for k in range(48)])
        Pbuy_data = np.array([self.extracted_values.get(f'Pbuy_{microgrid_id}_{k}', 0) for k in range(48)])
        Psell_data = np.array([self.extracted_values.get(f'Psell_{microgrid_id}_{k}', 0) for k in range(48)]) # 负值
        ...
        for j in range(self.num_microgrid):
            if j != microgrid_id:
                Pbuymic_data_dict[j] = np.array(
                    [self.extracted_values.get(f'Pbuymic_{microgrid_id}_{j}_{k}', 0) for k in range(48)])
        Psellmic_data_dict = {}
        for j in range(self.num_microgrid):
            if j != microgrid_id:
                Psellmic_data_dict[j] = np.array(
                    [self.extracted_values.get(f'Psellmic_{microgrid_id}_{j}_{k}', 0) for k in range(48)])
...
def MGO(C_buymic, C_sellmic, EVload):
    ...
    #======grid1========#
    id1 = 0
    load_1 = expand_array(microgrid_load_dict[id1])
    P_wt_1 = expand_array(microgrid_wt[id1])
    P_pv_1 = expand_array(microgrid_pv[id1])
    ...
    #======grid2========#
    id2 = 1
    load_2 = expand_array(microgrid_load_dict[id2])
    P_wt_2 = expand_array(microgrid_wt[id2])
    P_pv_2 = expand_array(microgrid_pv[id2])
    ...
    # 商业区域电网
    id3 = 2
    load_3 = expand_array(microgrid_load_dict[id3])
    P_wt_3 = expand_array(microgrid_wt[id3])
    P_pv_3 = expand_array(microgrid_pv[id3])
    ...
    # 商业区域电网
    id4 = 3
    load_4 = expand_array(microgrid_load_dict[id4])
    P_wt_4 = expand_array(microgrid_wt[id4])
    P_pv_4 = expand_array(microgrid_pv[id4])
